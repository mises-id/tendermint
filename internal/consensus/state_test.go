...
func TestStateProposerSelection0(t *testing.T) {
	...
	signAddVotes(cs1, tmproto.PrecommitType, config.ChainID(), time.Now(), types.BlockID{Hash: rs.ProposalBlock.Hash(), PartSetHeader: rs.ProposalBlockParts.Header()}, vss[1:]...)
...
func TestStateProposerSelection2(t *testing.T) {
	...
	for i := int32(0); int(i) < len(vss); i++ {
		...
		signAddVotes(cs1, tmproto.PrecommitType, config.ChainID(), time.Now(), types.BlockID{}, vss[1:]...)
...
func TestStateBadProposal(t *testing.T) {
	...
	signAddVotes(cs1, tmproto.PrevoteType, config.ChainID(), time.Now(), blockID, vs2)
	ensurePrevote(t, voteCh, height, round)

	// wait for precommit
	ensurePrecommit(t, voteCh, height, round)
	validatePrecommit(t, cs1, round, -1, vss[0], nil, nil)
	signAddVotes(cs1, tmproto.PrecommitType, config.ChainID(), time.Now(), blockID, vs2)
}
...
func TestStateLock_NoPOL(t *testing.T) {
	...
	*/
	...
	ensurePrevote(t, voteCh, height, round) // prevote
	blockID := types.BlockID{Hash: roundState.ProposalBlock.Hash(), PartSetHeader: roundState.LockedBlockParts.Header()}
	...
	hash := make([]byte, len(blockID.Hash))
	...
	*/
	...
	validatePrecommit(t, cs1, round, 0, vss[0], nil, blockID.Hash) // precommit nil but be locked on proposal
	...
	*/
	...
	ensureNewTimeout(t, timeoutWaitCh, height, round, cs1.config.Prevote(round).Nanoseconds())
	ensurePrecommit(t, voteCh, height, round)
	validatePrecommit(t, cs1, round, 0, vss[0], nil, blockID.Hash) // precommit nil but locked on proposal
...
func TestStateLock_POLUpdateLock(t *testing.T) {
	...
	*/
	...
	signAddVotes(cs1, tmproto.PrevoteType, config.ChainID(), time.Now(), blockID, vs2, vs3, vs4)
	...
	signAddVotes(cs1, tmproto.PrecommitType, config.ChainID(), time.Now(), blockID, vs2, vs3, vs4)
...
func TestStateLock_POLDoesNotUnlock(t *testing.T) {
	...
	*/
	...
	signAddVotes(cs1, tmproto.PrecommitType, config.ChainID(), time.Now(), blockID, vs2, vs4)
	signAddVotes(cs1, tmproto.PrecommitType, config.ChainID(), time.Now(), blockID, vs3)
...
func TestStateLock_MissingProposalWhenPOLSeenDoesNotUpdateLock(t *testing.T) {
	...
	*/
	...
	signAddVotes(cs1, tmproto.PrevoteType, config.ChainID(), time.Now(), firstBlockID, vs2, vs3, vs4)
	...
	signAddVotes(cs1, tmproto.PrecommitType, config.ChainID(), time.Now(), firstBlockID, vs2, vs3, vs4)
	...
	*/
	...
	signAddVotes(cs1, tmproto.PrevoteType, config.ChainID(), time.Now(), secondBlockID, vs2, vs3, vs4)

	ensurePrecommit(t, voteCh, height, round)
	validatePrecommit(t, cs1, round, 0, vss[0], nil, secondBlockID.Hash)
}
...
func TestStateLock_POLSafety1(t *testing.T) {
	...
	prevotes := signVotes(tmproto.PrevoteType, config.ChainID(), time.Now(),
		blockID,
		vs2, vs3, vs4)
...
func TestStateLock_POLSafety2(t *testing.T) {
	...
	// the others sign a polka but we don't see it
	prevotes := signVotes(tmproto.PrevoteType, config.ChainID(), time.Now(), propBlockID0, vs2, vs3, vs4)
	...
	propBlockID1 := types.BlockID{Hash: propBlockHash1, PartSetHeader: propBlockParts1.Header()}
	...
	signAddVotes(cs1, tmproto.PrevoteType, config.ChainID(), time.Now(), propBlockID1, vs2, vs3, vs4)
	...
	// add precommits from the rest
	signAddVotes(cs1, tmproto.PrecommitType, config.ChainID(), time.Now(), propBlockID1, vs2, vs4)
	signAddVotes(cs1, tmproto.PrecommitType, config.ChainID(), time.Now(), propBlockID1, vs3)
...
func TestState_PrevotePOLFromPreviousRound(t *testing.T) {
	...
	*/
	...
	rs := cs1.GetRoundState()
	blockID := types.BlockID{
		Hash:          rs.ProposalBlockParts.Hash(),
		PartSetHeader: rs.ProposalBlockParts.Header(),
	}
	...
	*/
	...
	signAddVotes(cs1, tmproto.PrevoteType, config.ChainID(), time.Now(), types.BlockID{}, vs2, vs3, vs4)
...
func TestSetValidBlockOnDelayedPrevote(t *testing.T) {
	...
	signAddVotes(cs1, tmproto.PrevoteType, config.ChainID(), time.Now(), blockID, vs3)
	...
	signAddVotes(cs1, tmproto.PrevoteType, config.ChainID(), time.Now(), blockID, vs4)
...
func TestWaitingTimeoutOnNilPolka(t *testing.T) {
	...
	signAddVotes(cs1, tmproto.PrecommitType, config.ChainID(), time.Now(), types.BlockID{}, vs2, vs3, vs4)
...
func TestStartNextHeightCorrectlyAfterTimeout(t *testing.T) {
	...
	signAddVotes(cs1, tmproto.PrecommitType, config.ChainID(), time.Now(), blockID, vs4)
...
func TestStateSlashing_Prevotes(t *testing.T) {
	...
	signAddVotes(tmproto.PrevoteType, hash,config.ChainID(),time.Now(), blockID, rs.ProposalBlock.Hash(),config.ChainID(), rs.ProposalBlockParts.Header(), vs2)
...
func TestStateSlashing_Precommits(t *testing.T) {
	...
	signAddVotes(tmproto.PrevoteType, rs.ProposalBlock.Hash(),config.ChainID(),time.Now(), blockID, waiting for more prevotes
	// add one for a different block should cause us to go into prevote wait
	hash := rs.ProposalBlock.Hash()
	hash[0] = byte(hash[0]+1) % 255
	signAddVotes(tmproto.PrecommitType, hash,config.ChainID(),time.Now(), blockID, rs.ProposalBlock.Hash(),config.ChainID(), rs.ProposalBlockParts.Header(), vs2)
...
func TestStateHalt1(t *testing.T) {
	...
	// add precommits from the rest
	signAddVotes(cs1, tmproto.PrecommitType, config.ChainID(), time.Now(), types.BlockID{}, vs2) // didnt receive proposal
	signAddVotes(cs1, tmproto.PrecommitType, config.ChainID(), time.Now(), blockID, vs3)
	// we receive this later, but vs3 might receive it earlier and with ours will go to commit!
	precommit4 := signVote(vs4, tmproto.PrecommitType, config.ChainID(), time.Now(), blockID)
...
func TestStateOutputVoteStats(t *testing.T) {
	...
	vote := signVote(vss[1], tmproto.PrecommitType, config.ChainID(), time.Now(), blockID)
